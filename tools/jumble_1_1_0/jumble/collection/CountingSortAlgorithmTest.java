...
public class CountingSortAlgorithmTest extends TestCase 
{
    ...
    // use this to perform verbose testing, with lots of output
    int verbose = 1;
    int repeatCount = 25;
	...
	{
		return new TestSuite(tests.CountingSortAlgorithmTest.class);
	}
   ...
   public void testRandomValues20long()
	...
	for (int runs = 0; runs<numberOfRuns; runs++) {
	    ...
	    if (verbose == 1)
		System.out.println("Testing array of size " + input.length);
	    for (int i=0; i<input.length; i++)
		input[i] = rng.nextInt();
	    try {
		int[] output = alg.sort(input);
